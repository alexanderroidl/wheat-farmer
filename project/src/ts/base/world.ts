...
import TruckEntity from "../entities/friends/truck";
...
export default class World {
    ...
    public spawnTruck (pos: Vector): EntityInterface {
      // No position given
    ...
    public update (delta: number): void {
      ...
      if (Math.floor((Date.now() - this.createdAt) / 1000) % 1 === 0 && this._truckCount < 1) {
        ...
        // Assign target
        if (truck instanceof TruckEntity && truck.hasWaited !== true) {
          // Initial target is centered under the field
          truck.target = new Vector(10, 20);
        } else { // Final target is the left side of the screen
          truck.target = new Vector(-18, 20);
        }
      }
      ...
      for (const entity of this.entities) {
        ...
        if (entity.target !== null) {
          ...
          /*
                ...
                Make individual velocity / acceleration functions for each entity
                or something like entity.speedIsConstant, where speed is fixed.
